...
const MOOD_GUIDANCE = {
  // Current scrubber notches
  unfocused:
    'Their mind is scattered and they need something that helps them focus. Prefer calm, well-paced, single-host or two-host episodes that pull attention back together. Avoid chaotic, over-stimulating, or many-guest formats. Real, easy-to-find episodes beat a perfect obscure match — do not keep searching if you already have two solid options.',
  worried:
  ...
  // Older mood keys still present on saved history rows
  excited:
...
Search the web for real, currently available podcast episodes that fit. Verify each episode actually exists and that the link points at the episode or its show page. Prefer episodes released in the last two years unless an older one is a clearly better fit. Two searches is plenty; stop as soon as you have real matches.
...
export function extractEpisodes(message) {
  ...
  if (start === -1 || end === -1 || end < start) {
    console.error('No JSON array in model reply:', text);
