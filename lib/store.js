...
export async function listEpisodes({ rating, mood, history } = {}) {
  ...
  if (history) builder = builder.or('rating.not.is.null,opened_at.not.is.null');
  ...
  return [...rows].sort((a, b) => {
    ...
    return (
      new Date(b.rated_at ?? b.opened_at ?? b.created_at) -
      new Date(a.rated_at ?? a.opened_at ?? a.created_at)
    );
  });
}
