...
export async function rateEpisode(id, rating, feedback) {
  const patch = { rating, rated_at: new Date().toISOString() };
  // Only touch the column when a caller actually sent feedback, so a plain
  // re-rate doesn't clobber feedback saved on a previous call.
  if (feedback !== undefined) patch.feedback = feedback;
  return unwrap(await client().from('episodes').update(patch).eq('id', id).select().single());
}
